...
const Page = () => {
  ...
  const handleChange = (e) => {
    ...
    if (name === "image" && files) {
      ...
      // If the checkbox allows multiple selections (e.g., 'website' field)
      if (Array.isArray(formData[name])) {
        const updatedValues = checked
          ? [...formData[name], value] // Add the checked value
          : formData[name].filter((item) => item !== value); // Remove the unchecked value
  ...
  useEffect(() => {
    ...
    const fetchBlogDetails = async () => {
      try {
        // Replace with your API endpoint to fetch the blog details
        const response = await fetch(
          `${BASE_URL}blog/fetch_blog_by_id/${id}`
        );
        if (response.ok) {
          const blogData = await response.json();

          setFormData({
            image: blogData.data.image, // If the image is a URL, you may need to handle this separately
            title: blogData.data.title,
            subtitle: blogData.data.subtitle,
            author: blogData.data.author,
            minutes: blogData.data.minutes,
            category: blogData.data.category,
            sustainability: blogData.data.sustainability || false,
          });

          setDescription(blogData.data.description);
          console.log(description)

          // Set tags (if your API provides tags)
          console.log(blogData.data.tags.length);
          if (Array.isArray(blogData.data.tags) && blogData.data.tags.length > 0) {

            const nonEmptyTags = blogData.data.tags.filter(tag => tag.trim() !== "");
            if (nonEmptyTags.length > 0) {
            const formattedTags = blogData.data.tags.map((tag) => ({ id: tag, text: tag }));
            setTags(formattedTags);
            }
          }

          // If the image is fetched as a URL
          if (blogData.data.image) {
            setImage(`${BASE_URL}${blogData.data.image}`);
  ...
  const handleSubmit = async (e) => {
    ...
    tags.forEach((tag, index) => {
      data.append(`tags[${index}]`, tag.text);
    });

    try {
      // const response = await fetch('https://admin.everonn.in/blog/create_blog', {
      const response = await fetch(
        `${BASE_URL}blog/edit_blog/${id}`,
        {
          method: "POST",
          body: data,
        }
      ...
      router.push("/blog_dashboard");
        ...
        /*setFormData({
          ...
          author: "",
          minutes: "",
          website: [], // Reset to an empty array
          category: "",
          sustainability: false,
        });*/
      } else {
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        if (response.ok) {
          const result = await response.json();
          console.log(result);
          const imageUrl = result.url; // Assuming your API returns the image URL
